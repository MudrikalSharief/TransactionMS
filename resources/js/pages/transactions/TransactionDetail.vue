    ...
    <div>
        ...
        <v-card rounded="0" elevation="1" class="lgu-card mb-4">
            <v-card-title class="d-flex align-center pa-5">
                ...
                <v-spacer />
                <v-btn variant="text" @click="$router.push('/transactions')"
                    >Back</v-btn
                >
            </v-card-title>
        ...
        <v-card rounded="0" elevation="1" class="lgu-card mb-4" v-if="tx">
            ...
            <v-card-text class="pa-4">
                ...
                <v-alert
                    v-if="!(tx?.station_checklist || []).length"
                    type="info"
                    variant="tonal"
                    class="mb-3"
                >
                    ...
                    >
                        ...
                        <template v-slot:[`item.name`]="{ item }">
                            <v-tooltip :disabled="rowRelevant(item)" location="top" max-width="320">
                                ...
                                <v-chip
                                    v-if="item.pivot.is_required"
                                    rounded="0"
                                    size="small"
                                    class="ml-2"
                                    variant="tonal"
                                    color="error"
                                    ><v-icon start size="small">mdi-asterisk</v-icon>required</v-chip
                                >
                                <v-chip
                                    v-else
                                    rounded="0"
                                    ...
                                    variant="tonal"
                                    color="grey"
                                    ><v-icon start size="small">mdi-minus-circle</v-icon>optional</v-chip
                                >
                                            </template>
                            </div>
                            <div
                                class="text-caption"
                                v-if="item.definition.description"
                            >
        ...
        <v-card rounded="0" elevation="1" class="lgu-card mb-4" v-if="false">
            ...
            <v-card-text class="pa-4">
                ...
                <v-alert
                    v-if="!tx.current_step_fields?.length"
                    type="info"
                    variant="tonal"
                    class="mb-3"
                >
        ...
        <v-dialog v-model="remarksDialog" max-width="800">
            <v-card rounded="0">
                ...
                <v-card-actions class="justify-end">
                    <v-btn variant="text" @click="remarksDialog = false"
                        >Cancel</v-btn
                    >
                    <v-btn
                        color="grey-darken-3"
            ...
            <v-card rounded="0">
                ...
                <v-card-text>
                    ...
                    <v-expansion-panels variant="accordion">
                        <v-expansion-panel rounded="0" title="Add station info (optional)">
                            <template #text>
                                <StepInfoFields :fields="tx?.current_step_fields" :form="form" />
                                <div v-if="!(tx?.current_step_fields || []).length" class="text-caption text-medium-emphasis">
                                    No info fields on this station.
                ...
                <v-card-actions class="justify-end">
                    ...
                    <v-btn color="grey-darken-3" rounded="0" :loading="savingChecklist" @click="confirmCheck">Check item</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
...
function hydrateForm() {
    ...
    for (const row of tx.value?.current_step_fields ?? []) {
        ...
        if (def.type === "multiselect")
            next[def.code] = Array.isArray(val) ? val : [];
    ...
    try {
        ...
        for (const row of tx.value?.current_step_fields ?? []) {
            ...
            if (
                ...
                !Array.isArray(payloadFields[def.code])
            ) {
        ...
        error.value = e?.response?.data?.message || "Execute failed.";
    } finally {
        saving.value = false;
    }
}
...
async function checkRequirement(requirementId) {
    ...
    try {
        const res = await api.post(
            `/api/transactions/${route.params.id}/requirements/${requirementId}/check`,
        );
...
const forwardActions = computed(() =>
    (availableActions.value || []).filter((a) => !a.is_return_route),
);
const returnActions = computed(() =>
    (availableActions.value || []).filter((a) => !!a.is_return_route),
);
