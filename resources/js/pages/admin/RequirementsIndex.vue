            ...
            <v-card rounded="0" class="lgu-card">
                <v-card-title class="d-flex align-center pa-5">
                    ...
                    <div class="flex-grow-1">
                        <div class="text-h6 font-weight-bold">Bind stations: {{ bindTarget?.name }}</div>
                        <div class="d-flex align-center ga-2 mt-1">
                            <v-chip size="x-small" variant="tonal" color="grey-darken-3" rounded="0" class="font-weight-bold">{{ bindTarget?.code }}</v-chip>
                            <span class="text-caption text-medium-emphasis">{{ bindRows.length }} station(s) bound</span>
                        </div>
                    </div>
                </v-card-title>
                <v-divider />
                <v-card-text class="pa-4">
                    ...
                    <v-autocomplete
                        ...
                        hint="Type to filter by order, name or code"
                        persistent-hint
                        multiple
                        chips
                        closable-chips
                    ...
                    <div class="d-flex align-center mb-1">
                        <div class="text-subtitle-2 font-weight-bold">Per-station overrides</div>
                        <v-spacer />
                        <span class="text-caption text-medium-emphasis">Lower order shows first to workers</span>
                    </div>
                    <div class="text-caption text-medium-emphasis mb-3">Order + required / optional per station.</div>
                    <v-alert v-if="!bindRows.length" type="info" variant="tonal" rounded="0">
                        Not bound to any station — pick at least one above, or save empty to unbind everywhere.
                    </v-alert>
                    <v-sheet v-else border rounded="0" class="overflow-hidden">
                    <v-data-table
                        :headers="bindHeaders"
                        :items="bindRows"
                    ...
                    >
                        <template v-slot:[`item.label`]="{ item }">
                            <div class="d-flex align-center ga-2 py-2">
                                <v-chip size="small" variant="tonal" color="grey-darken-3" rounded="0" class="font-weight-bold">{{ item.order_number }}</v-chip>
                                <div>
                        ...
                        <template v-slot:[`item.display_order`]="{ item }">
                            <v-text-field
                                v-model.number="bindOverrides[String(item.workflow_step_id)].display_order"
                                type="number"
                                min="0"
                                density="compact"
                                variant="outlined"
                        ...
                        <template v-slot:[`item.is_required`]="{ item }">
                            <div class="d-flex align-center ga-2">
                                <v-switch
                                    v-model="bindOverrides[String(item.workflow_step_id)].is_required"
                                    color="warning"
                                    inset
                                    density="compact"
                                    hide-details
                                    @click.stop
                                />
                                <span class="text-caption font-weight-bold" :class="bindOverrides[String(item.workflow_step_id)]?.is_required ? 'text-warning' : 'text-medium-emphasis'">
                                    {{ bindOverrides[String(item.workflow_step_id)]?.is_required ? "Required" : "Optional" }}
                                </span>
                            </div>
                        </template>
                    </v-data-table>
...
// Persistent per-station edits. bindRows is derived from this so toggles/inputs never reset.
...
function ensureBindOverrides() {
    const next = { ...(bindOverrides.value || {}) };
    for (const sid of (bindStationIds.value || [])) {
        const key = String(sid);
        if (!next[key]) {
            const pivot = pivotFor(sid);
            next[key] = {
...
function openBindDialog(item) {
    ...
    for (const s of (item?.steps || [])) {
        const pivot = s?.pivot_meta || s?.pivot || {};
